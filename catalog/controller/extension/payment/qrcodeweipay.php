...
require_once(DIR_APPLICATION."controller/extension/payment/WxPayPubHelper/WxPayPubHelper.php");
...
class ControllerExtensionPaymentQrcodeweipay extends Controller
{
    ...
    $data['button_confirm'] = $this->language->get('button_confirm');
        ...
        */
        ...
        if (is_file('../../../../callback/qrcodeweipay_callback.php')) {
            $unifiedOrder->setParameter("notify_url",HTTP_SERVER . 'callback/qrcodeweipay_callback.php');//通知地址
        } else {
            //通知地址, 该方式需要配置伪静态规则，且要把文件callback/qrcodeweipay_callback.php删除
            $unifiedOrder->setParameter("notify_url",HTTP_SERVER . 'payment_callback/qrcodeweipay');
        }
        ...
        */
        $data['redirect'] = $this->url->link('extension/payment/qrcodeweipay_qrcode');
    ...
    //使用通用通知接口
        ...
        //存储微信的回调
        if (isset($_POST['weipay_param'])) {
            $xml = $_POST['weipay_param'];
        } else {
