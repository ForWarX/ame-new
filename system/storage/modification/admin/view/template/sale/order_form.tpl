	...
	$.ajax({
		...
		success: function(json) {
			...
			if (json['error']) {
				...
				// Shipping Methods
				var request = $.ajax({
                //  url: '<?php echo $catalog; ?>index.php?route=api/shipping/methods&order_id=<?php echo $order_id; ?>&customer_group_id=' + $('select[name=\'customer_group_id\']').val()+'&token=' + token + '&store_id=' + $('select[name=\'store_id\'] option:selected').val(),

                  url: '<?php echo $catalog; ?>index.php?route=api/shipping/methods&order_id=<?php echo $order_id; ?>&customer_group_id=' + $('select[name=\'customer_group_id\']').val()+'&' + token + '&store_id=' + $('select[name=\'store_id\'] option:selected').val(),
					dataType: 'json',
					beforeSend: function() {
						$('#button-shipping-address').button('loading');
					},
