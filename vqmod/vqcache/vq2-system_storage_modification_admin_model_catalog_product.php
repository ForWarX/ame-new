...
// Begin WaterMark - from the modification "WaterMark by iSenseLabs"
                if (in_array(__FUNCTION__, array('addProduct', 'editProduct', 'deleteProduct'))) {
                    ...
                    $this->model_extension_module_iwatermark->handleEvent(__FUNCTION__, $product_id);
                }
...
/* 原有 导入下单产品
	public function addProductImport($data) {
        ...
        if (!empty($data['brand'])) {
            ...
            $query = $this->model_catalog_manufacturer->getManufacturerByName($data['brand']);
            if (!empty($query)) {
        ...
        $meta_description = json_encode(array('hscode' => $data['hscode'], 'taxnumber' => $data['taxnumber']));

		// 英文名和描述
        if (!empty($data['name_en'])) {
            $meta_title = $data['name_en'];
		    $this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '1', name = '" . $this->db->escape($data['name_en']) . "', description = '" . $data['desc_en'] . "', tag = '" . $this->db->escape($data['tag']) . "', meta_title = '" . $this->db->escape($meta_title) . "', meta_description = '" . $this->db->escape($meta_description) . "', meta_keyword = ''");
        }
		// 中文名和描述
		if (!empty($data['name_cn'])) {
            ...
            $meta_title = $data['name_cn'];
            $this->db->query("INSERT INTO " . DB_PREFIX . "product_description SET product_id = '" . (int)$product_id . "', language_id = '" . $language_id ."', name = '" . $this->db->escape($data['name_cn']) . "', description = '" . $data['desc_cn'] . "', tag = '" . $this->db->escape($data['tag']) . "', meta_title = '" . $this->db->escape($meta_title) . "', meta_description = '" . $this->db->escape($meta_description) . "', meta_keyword = ''");
        }
...
// Begin WaterMark - from the modification "WaterMark by iSenseLabs"
    ...
    public function editProductImport($product_id, $data) {
	    $sql = "UPDATE " . DB_PREFIX . "product SET upc = '" . $this->db->escape($data['upc']);
	    if (!empty($data['price'])) $sql .= "', price = '" . (float)$data['price'];
        if (!empty($data['weight'])) $sql .= "', weight = '" . (float)$data['weight'];
        if (!empty($data['length'])) $sql .= "', length = '" . (float)$data['length'];
        if (!empty($data['width'])) $sql .= "', width = '" . (float)$data['width'];
        if (!empty($data['height'])) $sql .= "', height = '" . (float)$data['height'];
        ...
        // 英文名和描述
        if (isset($data['name_en']) || isset($data['desc_en']) || isset($data['tag'])) {
        ...
        if (!empty($data['category'])) {
            ...
            if (!empty($query)) {
                $category_id = $query['category_id'];
                $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . (int)$product_id . "' AND category_id = '" . $category_id . "'");
                if (empty($query->row)) {
                    $this->db->query("INSERT INTO " . DB_PREFIX . "product_to_category SET product_id = '" . (int)$product_id . "', category_id = '" . $category_id . "'");
                }
            }
