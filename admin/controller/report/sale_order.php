...
class ControllerReportSaleOrder extends Controller {
	public function index() {
		...
		$data['import_url'] = $this->url->link('report/sale_order/import', 'token=' . $this->session->data['token'], true);
	...
	public function import() {
		if (!empty($this->request->files['file']['name']) && is_file($this->request->files['file']['tmp_name'])) {
			$file_ext = strtolower(substr(strrchr($this->request->files['file']['name'], '.'), 1));
			if ($file_ext != 'csv') {
				$data['message'] = 'Filename type error';
			} else {
				// Check to see if any PHP files are trying to be uploaded
				$content = file_get_contents($this->request->files['chinaid_back']['tmp_name']);
				$row = 1;
				if (($handle = fopen($this->request->files['chinaid_back']['tmp_name'], "r")) !== FALSE) {
			...
			$data['message'] = 'Unknow file';
		}
		
		$data['url'] = $this->url->link('report/sale_order', 'token=' . $this->session->data['token'], true);
	
		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('report/sale_order_export', $data));
	}
	...
	public function export() {
		...
		$results = $this->model_report_sale->getExportOrders($filter_data);
