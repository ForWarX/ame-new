...
class ControllerExtensionPaymentQrcodeweipay extends Controller
{
    ...
    public function index()
    {
        ...
        if (isset($this->error['qrcodeweipay_key'])) {
           ...
           $data['breadcrumbs'][] = array(
               'href'      => HTTPS_SERVER . 'index.php?route=extension/extension&token=' . $this->session->data['token'] . '&type=payment',
               'text'      => $this->language->get('text_payment'),
              'separator' =>' > '
           );

           $data['breadcrumbs'][] = array(
               'href'      => HTTPS_SERVER . 'index.php?route=extension/payment/qrcodeweipay&token=' . $this->session->data['token'],
        ...
        $data['action'] = HTTPS_SERVER . 'index.php?route=extension/payment/qrcodeweipay&token=' . $this->session->data['token'];
        ...
        if (isset($this->request->post['qrcodeweipay_total'])) {
            $data['qrcodeweipay_total'] = $this->request->post['qrcodeweipay_total'];
        } else {
            $data['qrcodeweipay_total'] = $this->config->get('qrcodeweipay_total');
        }
        ...
        $this->response->setOutput($this->load->view('extension/payment/qrcodeweipay', $data));
    }


    private function validate()
    {
        if (!$this->user->hasPermission('modify', 'extension/payment/qrcodeweipay')) {
