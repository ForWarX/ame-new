<?php
class ControllerInformationTrackings extends Controller {
	private $error = array();

	public function index() {
		$this->load->language('information/trackings');
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->response->redirect($this->url->link('information/trackings/success'));
		}
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('information/trackings', $url)
		);
		...
		$data['action'] = $this->url->link('information/trackings', '', true);
		...
		$data['headerings'] = $this->load->controller('common/headerings');
		//新增 TRACK 信息
		$data['text_ame_no'] = $this->language->get('text_ame_no');
...
//添加AME号
		if (isset($this->request->get['ame_no'])) {
			$ame_no = $this->request->get['ame_no'];
		} else {
			$ame_no = '';
		}
		$url = '';
		...
		if (isset($this->request->get['ame_no'])) {
			$url .= '&ame_no=' . urlencode(html_entity_decode($this->request->get['ame_no'], ENT_QUOTES, 'UTF-8'));
		}
		...
		$result = $this->model_account_order->getTrackOrder($ame_no);
		if($result){
		...
		$this->response->setOutput($this->load->view('information/trackings', $data));
	}
	...
	public function success() {
		...
		$data['breadcrumbs'][] = array(
			'text' => $this->language->get('heading_title'),
			'href' => $this->url->link('information/trackings')
		);
