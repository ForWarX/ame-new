	...
	public function index() {
		if (isset($this->request->get['token']) && isset($this->session->data['token']) && ($this->request->get['token'] == $this->session->data['token'])) {
			...
			*/	
			...
			if ($this->user->hasPermission('access', 'localisation/storage')) {
				$localisation[] = array(
						'name'	   => $this->language->get('text_storage'),
						'href'     => $this->url->link('localisation/storage', 'token=' . $this->session->data['token'], true),
						'children' => array()
				);
			}
