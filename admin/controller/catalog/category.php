...
class ControllerCatalogCategory extends Controller {
	...
	protected function getForm() {
        ...
        $data['entry_canMix'] = $this->language->get('entry_canMix');
        $data['entry_tax'] = $this->language->get('entry_tax');
        $data['entry_tax_limit'] = $this->language->get('entry_tax_limit');
        $data['entry_tax_type'] = $this->language->get('entry_tax_type');
        $data['entry_weight_limit'] = $this->language->get('entry_weight_limit');
        $data['entry_quantity_limit'] = $this->language->get('entry_quantity_limit');
        $data['entry_violation_fee'] = $this->language->get('entry_violation_fee');

		$data['help_filter'] = $this->language->get('help_filter');
		$data['help_keyword'] = $this->language->get('help_keyword');
		$data['help_top'] = $this->language->get('help_top');
		$data['help_column'] = $this->language->get('help_column');
        $data['help_canMix'] = $this->language->get('help_canMix');
        $data['help_tax_limit'] = $this->language->get('help_tax_limit');
        $data['help_violation_fee'] = $this->language->get('help_violation_fee');
        ...
        if (isset($this->request->post['canMix'])) {
            $data['canMix'] = $this->request->post['canMix'];
        } elseif (!empty($category_info)) {
            $data['canMix'] = $category_info['canMix'];
        } else {
            $data['canMix'] = 0;
        }
        ...
        if (isset($this->request->post['tax'])) {
            ...
            $data['tax'] = $category_info['tax'];
        } else {
            $data['tax'] = 0;
        }

        if (isset($this->request->post['tax_limit'])) {
            $data['tax_limit'] = $this->request->post['tax_limit'];
        } elseif (!empty($category_info)) {
            $data['tax_limit'] = $category_info['tax_limit'];
        } else {
            $data['tax_limit'] = 0;
        }
        ...
        if (isset($this->request->post['tax_type'])) {
            ...
            $data['tax_type'] = $category_info['tax_type'];
        } else {
            $data['tax_type'] = true;
        }

        if (isset($this->request->post['weight_limit'])) {
            $data['weight_limit'] = $this->request->post['weight_limit'];
        } elseif (!empty($category_info)) {
            $data['weight_limit'] = $category_info['weight_limit'];
        } else {
            $data['weight_limit'] = 0;
        }

        if (isset($this->request->post['quantity_limit'])) {
            $data['quantity_limit'] = $this->request->post['quantity_limit'];
        } elseif (!empty($category_info)) {
            $data['quantity_limit'] = $category_info['quantity_limit'];
        } else {
            $data['quantity_limit'] = 0;
        }

        if (isset($this->request->post['violation_fee'])) {
            $data['violation_fee'] = $this->request->post['violation_fee'];
        } elseif (!empty($category_info)) {
            $data['violation_fee'] = $category_info['violation_fee'];
        } else {
            $data['violation_fee'] = 0;
        }
