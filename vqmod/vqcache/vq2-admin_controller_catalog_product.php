...
/////////////**************end of code *********///////
        ...
        // 报关税率
        $bao_guan_tax =  ((float)$this->config->get('bao_guan_tax_rate')) * (1 + (float)$this->config->get('bao_guan_tax'));
		...
		foreach ($results as $result) {
			...
			$data['products'][] = array(
				...
				'model'      => $result['model'],
				'price'      => $result['price'],
				'bao_guan'   => round($result['price'] * $bao_guan_tax, 2),
				'special'    => $special,
	...
	protected function getForm() {
		...
		$data['entry_price'] = $this->language->get('entry_price');
		$data['entry_bao_guan'] = $this->language->get('entry_bao_guan');
		...
		// 报关税率
        $data['bao_guan_tax'] =  ((float)$this->config->get('bao_guan_tax_rate')) * (1 + (float)$this->config->get('bao_guan_tax'));
    ...
    public function import() {
        ...
        */
        ...
        $data['action_url'] = str_replace('&amp;', '&', $this->url->link('catalog/product/process_import', 'token=' . $this->session->data['token'], true));
    ...
    public function process_import() {
        ...
        if (empty($data['error'])) {
            if (($handle = fopen($tmp_name, "r")) !== FALSE) {
                $import_type = $this->request->post['upload_type']; // 导入方式：添加或覆盖
                ...
                while (($csv_data = fgetcsv($handle)) !== FALSE) {
                    ...
                    $desc_cn = $csv_data[3];        // 中文描述
                    $desc_en = $csv_data[4];        // 英文描述
                    $price = $csv_data[5];          // 单价（C$）
                    $length = $csv_data[6];         // 长(cm)
                    $width = $csv_data[7];          // 宽(cm)
                    $height = $csv_data[8];         // 高(cm)
                    $weight = $csv_data[9];         // 重量（kg）
                    $category = $csv_data[10];      // 品类名
                    $brand = $csv_data[11];         // 品牌
                    $tag = $csv_data[12];           // 规格
                    $quantity = empty($csv_data[13]) ? 1000 : $csv_data[13]; // 库存
                    $hscode = $csv_data[14];        // HS编码
                    $taxnumber = $csv_data[15];     // 行邮税号
                    ...
                    if (empty($product)) {
                        ...
                        $para['desc_cn'] = $desc_cn;
                        $para['desc_en'] = $desc_en;
                        $para['price'] = $price;
                        $para['length'] = $length;
                        $para['width'] = $width;
                        $para['height'] = $height;
                        ...
                        $para['hscode'] = $hscode;
                        $para['taxnumber'] = $taxnumber;
                        $this->model_catalog_product->addProductImport($para);
                    } elseif ($import_type == 'cover') {
                        // 覆盖商品信息
                        $para['upc'] = $upc;
                        $para['name_en'] = $name_en;
                        $para['name_cn'] = $name_cn;
                        ...
                        $para['hscode'] = $hscode;
                        $para['taxnumber'] = $taxnumber;
                        $this->model_catalog_product->editProductImport($product['product_id'], $para);
                    } else {
                        // 添加商品信息
                        $para['upc'] = $upc;
                        if (empty($product['name_en'])) $para['name_en'] = $name_en;
                        if (empty($product['name_cn'])) $para['name_cn'] = $name_cn;
                        if (empty($product['desc_cn'])) $para['desc_cn'] = $desc_cn;
                        if (empty($product['desc_en'])) $para['desc_en'] = $desc_en;
                        if ((float)$product['price'] == 0) $para['price'] = $price;
                        if ((float)$product['length'] == 0) $para['length'] = $length;
                        if ((float)$product['width'] == 0) $para['width'] = $width;
                        if ((float)$product['height'] == 0) $para['height'] = $height;
                        if ((float)$product['weight'] == 0) $para['weight'] = $weight;
                        $para['category'] = $category;
                        if (empty($product['manufacturer_id'])) $para['brand'] = $brand;
                        if (empty($product['tag'])) $para['tag'] = $tag;
                        if ((int)$product['weight'] == 0) $para['quantity'] = $quantity;
                        if (empty($product['hscode'])) $para['hscode'] = $hscode;
                        if (empty($product['taxnumber'])) $para['taxnumber'] = $taxnumber;
                        $this->model_catalog_product->editProductImport($product['product_id'], $para);
                    }
                }
