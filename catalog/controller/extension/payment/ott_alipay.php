...
class ControllerExtensionPaymentOTTAlipay extends Controller {
	...
	// }
	// checkout page alipay option
	public function index() {
		$data['button_confirm'] = $this->language->get('button_confirm');

		$data['redirect'] = $this->url->link('extension/payment/ott_alipay/qrcode');

		return $this->load->view('extension/payment/ott_alipay', $data);
	}
	...
	public function qrcode() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('extension/payment/ott_alipay/qrcode')
		);
		/* gather infos*/
		/*
		info list:
		...
		*/
		...
		//    $dataDate = date('YmdHis').'123';
		// var_dump($dataDate);
		// echo mb_substr($dataDate,14);
		...
		/* No. 3*/
		$notify_url = "http://americo.itecht.ca/americoAlipay";//$this->url->link('extension/payment/ott_alipay/callback');
		$call_back_url = $notify_url;
	//----start
	// $myFile = "alipay_url_checking.txt";
	// $fh = fopen($myFile, 'a+') or die("can't open file");
	// $stringData = $call_back_url;
	// fwrite($fh, "\nrecord date: " .date('YmdHis')." \n record message: ".$stringData."\n");
	// fclose($fh);
	//----end
		/* No. 4*/
		$biz_type = "ALIPAY";
		...
		/* No. 7*/
		$mch_id = $this->config->get('ott_alipay_merchant_id');
		$merchant_id = $mch_id;
		
		/* No. 8*/		
		$sign_key = $this->config->get('ott_alipay_sign_key');
		$key = $sign_key;


		/* modify data */
		$aliPaymentInfo = new AliPaymentInfo();
		$aliSecurity = new AliSecurity();
		$md5hash = $aliPaymentInfo->organizeInfo($orderId,$call_back_url,$biz_type,$amount); 
		$aesMd5String = md5($md5hash.$key);
		$aesStringRaw = $aliPaymentInfo->my_substr_function($aesMd5String,8,24);
		...
		$aesClass = new AES($dataBlockJson, $aesKey,128,'ECB');
		...
		if (!isset($this->session->data['ott_alipay_orderId'] )){
			$this->session->data['ott_alipay_orderId']  = "";
		  }
		if($this->session->data['ott_alipay_orderId'] != $orderId){
			//set session order id
			$this->session->data['ott_alipay_orderId'] = $orderId;
			//

			if($isSendHttp){
	
				$ottResponseJson = $aliSecurity->toSendOttRequest($preSend);
				$resObj = json_decode($ottResponseJson);
	
				$resMd5hash = $resObj->md5;
				$resAesMd5String = md5($resMd5hash.$key);
				$resAesStringRaw = $aliPaymentInfo->my_substr_function($resAesMd5String,8,24);
				$resAesKey = mb_strtoupper($resAesStringRaw, "UTF-8");
	
				$aesClass = new AES($resObj->data, $resAesKey,128,'ECB');
				$resOutput = $aesClass->decrypt();
	
				$cleanOutput = $aliPaymentInfo->escape_sequence_decode($resOutput);
			...
			//set session code url for new qr code
			$this->session->data['ott_alipay_code_url'] = $data['code_url'];
		} else {
			//same order id
			//set code_url from session
			$data['code_url'] = $this->session->data['ott_alipay_code_url'];
		}
		...
		$this->response->setOutput($this->load->view('extension/payment/ott_alipay_qrcode', $data));
	}
	...
	public function isOrderPaid() {
		...
		if (isset($this->request->get['order_id'])) {
			...
			if ($order_info['order_status_id'] == $this->config->get('ott_alipay_completed_status_id')) {
				$json['result'] = true;
			}
		}
	...
	// 		if (!$order_status_id) {
	// 			// echo 'order_status_id<br>' . $order_status_id;
	// 			$this->model_checkout_order->addOrderHistory($resOrderId, $this->config->get('ott_alipay_completed_status_id'));
	// 		}
	// 	}
	// }
	...
	public function callback() {
		...
		$decoded  = json_decode( $raw_post );
	//----start
	$myFile = "ott_alipay_callback_log.txt";
	$fh = fopen($myFile, 'a+') or die("can't open file");
	$stringData = $raw_post;
	fwrite($fh, "\nrecord date: " .date('YmdHis')." \n record message: ".$stringData."\n");
	fclose($fh);
	//----end
		...
		$key =$this->config->get('ott_alipay_sign_key');
		// echo 'key: ' . $key;
		$resAesMd5String = md5($resMd5hash.$key);
		$aliPaymentInfo = new AliPaymentInfo();
		$aliSecurity = new AliSecurity();
		$resAesStringRaw = $aliPaymentInfo->my_substr_function($resAesMd5String,8,24);
		$resAesKey = mb_strtoupper($resAesStringRaw, "UTF-8");
		$aesClass = new AES($resData, $resAesKey,128,'ECB');
		$resOutput = $aesClass->decrypt();
		$cleanOutput = $aliPaymentInfo->escape_sequence_decode($resOutput);
		$cleanOutputObj = json_decode($cleanOutput); 
		//----------- get order id after decoding ----------------------
		//mb_substr($dataDate,14);
		if (isset($cleanOutputObj->order_id)) {
		} else {
		}
	    $resOrderId = mb_substr($cleanOutputObj->order_id,14);//$cleanOutputObj->order_id;
		//$resOrderId = '43';
		//----------- write response to local file -----------------------
		// $myFile = "ott_alipay_callback_log.txt";
		// $fh = fopen($myFile, 'a+') or die("can't open file");
		// $stringData = $raw_post;
		// fwrite($fh, "\n" .date('YmdHis').' '.$stringData."\n merchant_id: ".$this->config->get('ott_alipay_merchant_id'));
		// fclose($fh);
...
class AliPaymentInfo{
	...
	public function organizeInfo($order_Id,$call_back_url,$biz_type, $amountInput){
		...
		// $checkOrderId = AliPaymentInfo::checkOrderId($orderId);
		// //create order id for user
		// if($checkOrderId){
			
		// }else{
		//     $orderId= AliPaymentInfo::creatOrderId();
		// }
...
}// end \AliPaymentInfo class


//encrypt and decrypt functions are deprecated, not recommend to use
class AliSecurity {
	public static function encrypt($input, $key) {
	$size = mcrypt_get_block_size(MCRYPT_RIJNDAEL_128, MCRYPT_MODE_ECB);
	$input = AliSecurity::pkcs5_pad($input, $size);
