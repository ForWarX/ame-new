...
class ControllerSaleOrder extends Controller {
	...
	public function export() {
        ...
        $output = "客户单号,起运地,发件人,发件地址,发件人电话,发件人国别,发件人城市,收件人,收件人证件号,详细地址,收件人电话,货物sku编码,货物数量,货物品名,品名简称,英文名称,商品规格,HS编码,行邮税号,单品货值,单品毛重（kg）,币别,货物单位,单品法定申报数量,法定计量单位,总重（lb）,总价（C\$）\r\n";
        ...
        foreach ($results as $key => $result) {
            ...
            foreach ($products as $product) {
                $line = $result['invoice_prefix'] . ',加拿大,AME,"3445 Sheppard Ave E, Scarborough",,加拿大,多伦多,' . $result['shipping_firstname']
                    . ',' . $result['shipping_chinaid'] . ',' . $result['shipping_zone'] . "（省）" . $result['shipping_city'] . "（市）" . $result['shipping_address_1']
                    . ',' . $result['shipping_phone'] . ',' . $product['upc'] . ',' . $product['quantity'] . ',' . $product['name'] . ',,'
                    . $product['name'] . ',' . $product['tag'] . ',,,' . $product['price'] . ',' . $product['weight'] . ',RMB,,1,,' . $result['weight'] . ',' . $result['total'];
                $output .= $line . "\r\n";
            }
        }
